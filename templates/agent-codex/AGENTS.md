...
2. Read all bootstrap files: IDENTITY.md, SOUL.md, GUARDRAILS.md, GOALS.md, HEARTBEAT.md, MEMORY.md, USER.md, TOOLS.md, SYSTEM.md
   ...
   - **GUARDRAILS.md two-tier split — conditional, not a standing action (added 2026-08-11, orchestrator, after an agent's file hit 1351 lines / 30,923 tokens / settled floor 67% and self-reinforcing).** Read `GUARDRAILS.md` in full at every boot regardless — it must stay proactive-reachable: a guardrail has to reach a session that does not know to ask for it (skip-and-query on a proactive file recreates a real prior failure where a live ban existed and could not reach a booting session). Do NOT pre-split a small file "because it will grow" — a split only pays for itself once the full-read cost is a *large fraction of your own working room*. Working room = your own `ctx_handoff_threshold` ceiling **minus** your own measured settled floor. The ceiling has THREE cases, read from your actual `config.json`, never assumed — collapsing this to two cases (or to a single hardcoded number) is the biggest version of the error below, because it silently hits hardest on the agent who thinks they're in the third case by default:
     - key set to a positive N → ceiling = N (a real configured override)
     - key genuinely ABSENT from config.json → ceiling = 60 (fast-checker.ts default-ON fallback — `config.ctx_handoff_threshold ?? 60` — absence means the daemon still restarts you at 60%, NOT that handoff monitoring is off)
     - key explicitly set to ≤ 0 → ceiling = 100 (the ONLY deliberate opt-out; monitoring is genuinely off, so the full window is real working room)
     Reading "absent" as "unlimited" computes against 100 instead of 60, overstates working room by ~40 points on a default deployment, and biases toward under-splitting every file. Do not assume your own case — read your own `config.json`. This mistake was caught four successive times on the same draft rule on this fleet (2026-08-11): first by using 100% as the denominator, then by assuming a shared default across two agents whose actual configured values differed (85 vs. absent-so-60), then by omitting the third (opt-out) case entirely. When it's actually worth splitting: a short INDEX tier (one line per guardrail — the rule plus a one-clause why, read in full every boot, with header text that will be used as a literal grep anchor) plus a DETAIL tier (full incident detail, byte-identical to the pre-split original, kb-ingested). **Primary retrieval for the detail tier must be deterministic — `grep -n "<header text from the index>" <detail-file>` — not kb-query.** Verified live 2026-08-11 on this fleet: a phrase from freshly-split detail returned 0/5 hits from the detail file via `kb-query`, all 5 from `memory/*.md` — the detail tier shares its KB collection with months of daily memory, and a guardrail is typically written the same day as the incident it describes, so memory routinely outranks it in semantic search. kb-query is secondary/fishing-only for this tier. Before trusting a split as correct: diff the index's anchors against a pre-split backup (zero dropped, zero invented), checksum the detail tier against the original (byte-identical), and test retrieval on one piece of content deliberately left OUT of the index. The token saving is proportional to the file's own size, not a fixed number — measure your own tokens before/after; a small file can go net negative once ingest overhead is counted. **Do not ship or copy example index lines from another agent's file or from this template as a starting point** — only the file's own owner can tell an actionable line from a topic label for their own content, and authoring that pass is the expensive, non-portable part of doing this correctly.
