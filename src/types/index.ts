...
export interface AgentConfig {
  ...
  /**
   * Minutes after an outbound Telegram message during which the daemon will NOT
   * instruct a restarting session to announce itself again. Defaults to 10 when
   * absent. Set to 0 to disable suppression entirely and always announce.
   *
   * Exists because the boot announcement is generated by the daemon, so no
   * agent-level doc can suppress it: measured on this host, 326 of 425 messages
   * (77%) an agent had sent its owner were boot announcements, across 327
   * restarts in 11 days at a median gap of 12.7 minutes.
   */
  boot_message_suppression_minutes?: number;
}
